...
async function ensureStoreInitialized() {
  ...
  for (const voice of sampleVoices) {
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      const wavBlob = await bufferToWave(renderedBuffer, 0, renderedBuffer.length)
...
function bufferToWave(abuffer: AudioBuffer, offset: number, len: number): Promise<Blob> {
...
export async function createModel(voices: Record<string, string[]>, name: string) {
  await ensureStoreInitialized()
